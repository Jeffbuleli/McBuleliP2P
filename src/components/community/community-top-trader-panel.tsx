...
export function CommunityTopTraderPanel({ fr }: { fr: boolean }) {
  ...
  const [participant, setParticipant] = useState<TopTraderParticipantStatus | null>(
    null,
  );
  ...
  const loadMyTrades = useCallback(async () => {
    try {
      const res = await fetch("/api/community/top-trader/trades?limit=10");
      if (!res.ok) return;
      const j = await res.json();
  ...
  const loadTradesFor = async (handle: string | null, userId: string) => {
    ...
    try {
      const q = handle
        ? `?handle=${encodeURIComponent(handle)}&limit=15`
        : `?userId=${encodeURIComponent(userId)}&limit=15`;
  ...
  return (
    <div className="space-y-3">
      ...
      {participant?.optedIn && viewer ? (
        <div className="fd-card flex items-center justify-between px-4 py-3">
          <div>
      ...
      <section>
        ...
        {loading ? (
          ...
          <div className="fd-card flex flex-col items-center px-4 py-8 text-center">
                ...
                {expandedUserId === entry.userId ? (
                  <div className="mt-2 space-y-2 pl-2">
                    {tradesLoading ? (
                      ...
                      trades.map((t) => (
                        <CommunityTopTraderTradeCard key={t.id} fr={fr} trade={t} />
                      ))
                    )}
                  </div>
                ) : null}
