...
import { rejectMcbClaim } from "@/lib/mcb-claim-service";
...
export async function POST(
  ...
  const result = await rejectMcbClaim({
    claimId: id,
  ...
  await writePlatformAdminAudit({
    ...
    action: PlatformAdminAuditAction.MCB_CLAIM_REJECT,
    resourceType: "mcb_claim",
    resourceId: id,
    meta: { reason: parsed.data.reason.trim() },
  });
