...
echo ""
echo "==> 2. prosodyctl check (Duplicate option = cause probable)"
CHECK="$(prosodyctl check config 2>&1 || true)"
echo "$CHECK" | grep -iE 'Duplicate option|Error|Fatal' || echo "  (pas de Duplicate option)"
echo "$CHECK" | grep -i 'Duplicate option' && DUP=1 || DUP=0
CONF_N=$(grep -c "Component \"conference.${DOMAIN}\"" /etc/prosody/conf.d/${DOMAIN}.cfg.lua 2>/dev/null || echo 0)
echo "  blocs Component conference.${DOMAIN}: ${CONF_N} (attendu: 1)"
[[ "$CONF_N" -gt 1 ]] && echo "  FAIL: plusieurs blocs conference → fix-prosody-dedupe-cfg.sh"

echo ""
echo "==> 3. focus component (target_address EXACT)"
grep -A5 "Component \"${FOCUS_COMP}\"" /etc/prosody/conf.d/${DOMAIN}.cfg.lua 2>/dev/null || \
  echo "FAIL: composant absent"

echo ""
echo "==> 4. c2s auth COMPLET (compter focus@)"
...
echo ""
echo "==> 5. c2s live (1 seul onglet pour test isolé)"
...
echo ""
echo "==> 6. Jicofo Registered récent ?"
...
echo ""
echo "==> 7. prosody.log — service-unavailable / focus (50 dernières lignes pertinentes)"
...
echo ""
echo "==> 8. jicofo.conf client-proxy"
grep -E 'client-proxy|conference-muc-jid|hostname|domain|username' /etc/jitsi/jicofo/jicofo.conf 2>/dev/null | head -8

echo ""
echo "INTERPRÉTATION"
if [[ "${DUP:-0}" -eq 1 || "${CONF_N:-0}" -gt 1 ]]; then
  echo "  Duplicate option / blocs conference dupliqués → client_proxy IQ cassé"
  echo "  → sudo bash ops/jitsi/fix-prosody-dedupe-cfg.sh"
  echo "  → sudo bash ops/jitsi/fix-focus-pre-join.sh"
elif [[ "$FOCUS_N" -ge 1 && "$C2S_N" -ge 1 ]]; then
