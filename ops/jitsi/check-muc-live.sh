...
EXPECT
  ...
  if [[ "$MUC_OK" -eq 1 ]]; then
  ...
  elif [[ "$N" -ge 1 && "$SVC_UNAVAIL" -eq 1 ]]; then
    echo "  FOCUS FAIL: service-unavailable récent dans prosody.log"
    echo "  → sudo bash ops/jitsi/fix-focus-service-unavailable.sh"
    ...
    [[ "$PREJOIN_BAD" -eq 1 ]] && echo "  → prejoin true: fix-config-force-join.sh"
  elif [[ "$N" -ge 1 ]]; then
