...
-- muc:room() est une commande console, pas un global Lua — utiliser host.modules.muc.
...
local function muc_module(conf_host)
    local h = prosody.hosts and prosody.hosts[conf_host]
...
local function find_room(muc_mod, jid)
    ...
    if muc_mod.rooms and muc_mod.rooms[jid] then
        return muc_mod.rooms[jid]
    end
    return nil
end
...
for name in pairs(prosody.hosts or {}) do
...
for name, h in pairs(prosody.hosts or {}) do
