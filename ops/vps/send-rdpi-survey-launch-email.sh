#!/usr/bin/env bash
# Send RDPI survey launch email.
# Usage:
#   bash ops/vps/send-rdpi-survey-launch-email.sh --to hi@mcbuleli.org
#   bash ops/vps/send-rdpi-survey-launch-email.sh --to maristote@rdpithinktank.org
set -euo pipefail

REPO="${MCBULELI_REPO:-/opt/mcbuleli}"
COMPOSE_DIR="${MCBULELI_COMPOSE_DIR:-$REPO/ops/vps}"
TO="${1:---to}"
ADDR="${2:-hi@mcbuleli.org}"
...
if [[ -z "$RESEND" || "$ALLOW" != "true" ]]; then
  echo "ERROR: Resend not configured" >&2
  exit 1
fi

echo "==> $(date -u +%Y-%m-%dT%H:%M:%SZ) rdpi-survey-launch --to $ADDR --send"
...
docker run --rm --network "$NET" \
  ...
  bash -lc "set -e
    ...
    npx tsx scripts/send-rdpi-survey-launch-email.ts --to '$ADDR' --send
  "
