#!/usr/bin/env bash
# Seed + send Bienv Photography media confirmation email (prod).
# Usage:
#   bash ops/vps/send-bienv-photography-email.sh
#   bash ops/vps/send-bienv-photography-email.sh --to bienvngonda862@gmail.com
set -euo pipefail

REPO="${MCBULELI_REPO:-/opt/mcbuleli}"
COMPOSE_DIR="${MCBULELI_COMPOSE_DIR:-$REPO/ops/vps}"
TO="${BIENV_PHOTO_TO:-bienvngonda862@gmail.com}"
...
echo "==> $(date -u +%Y-%m-%dT%H:%M:%SZ) bienv-photography seed + send → $TO"
...
docker run --rm --network "$NET" \
  ...
  bash -lc "set -e
    ...
    npx tsx scripts/send-bienv-photography-email.ts --to ${TO} --send
  "
