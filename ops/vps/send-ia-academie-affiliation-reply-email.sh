#!/usr/bin/env bash
# Seed partner orgs + send IA Académie affiliation reply (logo + dual-email access).
# Usage:
#   bash ops/vps/send-ia-academie-affiliation-reply-email.sh
#   bash ops/vps/send-ia-academie-affiliation-reply-email.sh --to contact@ia-academie.cd
set -euo pipefail

REPO="${MCBULELI_REPO:-/opt/mcbuleli}"
COMPOSE_DIR="${MCBULELI_COMPOSE_DIR:-$REPO/ops/vps}"
TO="${IA_ACADEMIE_REPLY_TO:-contact@ia-academie.cd}"
...
echo "==> $(date -u +%Y-%m-%dT%H:%M:%SZ) ia-academie affiliation reply → $TO (cc contact@ch-kin.com)"
...
docker run --rm --network "$NET" \
  ...
  bash -lc "set -e
    ...
    npx tsx scripts/send-ia-academie-affiliation-reply-email.ts --to ${TO} --send
  "
