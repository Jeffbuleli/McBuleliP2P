# RDPI Think Tank × McBuleli Hackathon

- **Site** : https://rdpithinktank.org/
- **Contact** : info@rdpithinktank.org
- **Référent** : Aristote MUGISHO (DG) · maristote@rdpithinktank.org
- **Profil** : Policy & Think Tank · recherche appliquée · politiques publiques · impact socio-économique

## Rôle confirmé

**Partenaire Policy & Impact**

## Contributions confirmées

1. Atelier : innovation, politiques publiques & impact socio-économique en RDC
2. Mentorat : impact, adoption des innovations & enjeux réglementaires
3. Jury : pertinence socio-économique, durabilité & potentiel d'impact
4. Diffusion via canaux institutionnels

## RDV

**Confirmé** : vendredi **24 juillet 2026 à 10h30** (Kinshasa) - visio McBuleli Meet
- Lien Meet : https://mcbuleli.org/meet/rdpi-thinktank-partenariat
- Email Meet : `content/email-partnership/rdpi-thinktank-meet.{html,txt}`
- Script : `npm run email:rdpi-thinktank-meet`
- Rappel début RDV : `npm run email:rdpi-thinktank-meet-reminder`
- Prod : to `maristote@rdpithinktank.org` · CC `info@rdpithinktank.org` (+ ceo)

## Email proposition initiale

- `content/email-partnership/vision-partners/rdpi-thinktank.{html,txt}`
