# IA Académie RDC / CHK × McBuleli Hackathon

## Statut

**Confirmé (intérêt)** - Rodrigue KASHARA DAVID (DG), réponse positive en tant que **partenaire académique**.

Sites : [ch-kin.com](https://ch-kin.com/) · [i-academie.com](https://i-academie.com/)  
Contacts : contact@ch-kin.com · contact@ia-academie.cd · (+243) 90 18 15 632

## Dates prévues (lieu en attente Silikin)

- 12 août 2026 - Jour 1 Bootcamp (08h00-13h30)
- 13 août 2026 - Jour 2 Build Day (08h00-13h30)
- 14 août 2026 - Jour 3 Demo Day (08h00-13h30)

**Statut** : en attente d'approbation finale de Silikin Village.

## Rôle validé

Partenaire académique :
- mobilisation apprenants / alumni
- atelier ou sessions de mentorat
- éventuelle participation au jury
- logo + visibilité sur `/hackathon`

## Prochaine étape (async, sans appel)

Email HTML fiche : `content/email-partnership/ia-academie-chk-fiche-partenariat.html`

```bash
npm run email:ia-academie-chk-fiche -- --to hi@mcbuleli.org --send
npm run email:ia-academie-chk-fiche -- --to contact@ch-kin.com --send
```

Demander :
1. Contact référent
2. Logo
3. Confirmation contributions (vivier / atelier / jury)