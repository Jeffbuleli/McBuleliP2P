/**
 * Kilelo partnership - preview / send via Resend.
 *
 *   npx tsx scripts/send-kilelo-email.ts --preview
 *   npx tsx scripts/send-kilelo-email.ts --to hi@mcbuleli.org --send
 *   npx tsx scripts/send-kilelo-email.ts --to support@kileloapp.com --send
 */
import { existsSync, mkdirSync, writeFileSync } from "node:fs";
import path from "node:path";
import { loadEnvFile } from "node:process";
import {
  buildKileloPartnershipEmail,
  KILELO_EMAIL,
} from "../src/lib/email/partnership/kilelo-email";
import {
  canSendViaResendApi,
...
async function main() {
  ...
  const email = buildKileloPartnershipEmail();
  const outDir = path.join(process.cwd(), "content/email-partnership");
  mkdirSync(outDir, { recursive: true });

  writeFileSync(path.join(outDir, "kilelo-partnership.html"), email.html, "utf8");
  writeFileSync(path.join(outDir, "kilelo-partnership.txt"), email.text, "utf8");
  console.log(`Subject: ${email.subject}`);
  console.log("✓ HTML → content/email-partnership/kilelo-partnership.html");

  if (!args.send) {
    console.log(`\nTest: npx tsx scripts/send-kilelo-email.ts --to ${SUPPORT_EMAIL} --send`);
    console.log(`Prod:  npx tsx scripts/send-kilelo-email.ts --to ${KILELO_EMAIL} --send`);
    return;
  }
