/**
 * ISTA campus ambassador — seed ISTA-KIN + preview / send.
 *
 *   npx tsx scripts/send-ista-ambassador-email.ts
 *   npx tsx scripts/send-ista-ambassador-email.ts --to hi@mcbuleli.org --send
 *   npx tsx scripts/send-ista-ambassador-email.ts --to coordinationestudiantinedeista@gmail.com --send
 */
import { existsSync, mkdirSync, writeFileSync } from "node:fs";
import path from "node:path";
import { loadEnvFile } from "node:process";
import { buildIstaAmbassadorEmail } from "../src/lib/email/partnership/ista-ambassador-email";
...
import {
  ISTA_AMB_EMAIL,
  ensureIstaAmbassadorAssets,
} from "../src/lib/hackathon/ista-ambassador";
...
async function main() {
  ...
  const assets = await ensureIstaAmbassadorAssets();
  const email = buildIstaAmbassadorEmail(assets);

  const outDir = path.join(process.cwd(), "content/email-partnership");
  mkdirSync(outDir, { recursive: true });
  writeFileSync(path.join(outDir, "ista-ambassador.html"), email.html, "utf8");
  writeFileSync(path.join(outDir, "ista-ambassador.txt"), email.text, "utf8");

  console.log(`Subject: ${email.subject}`);
  console.log(`Promo: ${assets.promoCode}`);
  console.log(`Share: ${assets.shareUrl}`);
  console.log(`Dashboard: ${assets.dashboardUrl}`);
  console.log("✓ HTML → content/email-partnership/ista-ambassador.html");
  ...
  if (!args.send) {
    console.log(
      `\nTest: npx tsx scripts/send-ista-ambassador-email.ts --to ${SUPPORT_EMAIL} --send`,
    );
    console.log(
      `Prod: npx tsx scripts/send-ista-ambassador-email.ts --to ${ISTA_AMB_EMAIL} --send`,
    );
    return;
  }
