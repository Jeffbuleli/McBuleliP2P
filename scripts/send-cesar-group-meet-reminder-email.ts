/**
 * César Group - rappel début RDV McBuleli Meet (14h00 Kinshasa).
 *
 *   npx tsx scripts/send-cesar-group-meet-reminder-email.ts --preview
 *   npx tsx scripts/send-cesar-group-meet-reminder-email.ts --to hi@mcbuleli.org --send
 *   npx tsx scripts/send-cesar-group-meet-reminder-email.ts --to cesargrouprdc@gmail.com --send
 */
...
const SUBJECT =
  "Rappel - RDV César Group maintenant (14h00) · McBuleli Meet";
...
function loadDossier() {
  ...
  return {
    html: readFileSync(path.join(base, "cesar-group-meet-reminder.html"), "utf8"),
    text: readFileSync(path.join(base, "cesar-group-meet-reminder.txt"), "utf8"),
  };
}
...
async function main() {
  ...
  console.log(`HTML: content/email-partnership/cesar-group-meet-reminder.html`);
  console.log(
  ...
  if (!args.send) {
    console.log(
      `\nTest: npx tsx scripts/send-cesar-group-meet-reminder-email.ts --to ${SUPPORT_EMAIL} --send`,
    );
    console.log(
      `Prod: npx tsx scripts/send-cesar-group-meet-reminder-email.ts --to ${CESAR_MEET_TO} --send`,
    );
    return;
  }
